...
import { uploadBase64ImageToS3 } from "./uploadController.js";
...
export const createCategory = async (req, res) => {
  try {
    ...
    if (logo) {
      categoryData.logo = await uploadBase64ImageToS3(logo, "uploads/logos");
    }

    if (image) {
      categoryData.image = await uploadBase64ImageToS3(image, "uploads/images");
    }
...
export const updateCategory = async (req, res) => {
  try {
    ...
    if (logo) {
      category.logo = await uploadBase64ImageToS3(logo, "uploads/logos");
    }

    if (image) {
      category.image = await uploadBase64ImageToS3(image, "uploads/images");
    }
